...
static VOID LoadMaterials( in1GEOM *G, CHAR *FileName )
{
  ...
  DefMat.Ka = ShaderVecTransfrov(VecSet(0.1, 0.1, 0.1));
  DefMat.Kd = ShaderVecTransfrov(VecSet(0.9, 0.9, 0.9));
  DefMat.Ks = ShaderVecTransfrov(VecSet(0.0, 0.0, 0.0));
  DefMat.Phong = 30;
  DefMat.TexNo = 0;
  DefMat.Trans = 1;
...
BOOL IN1_GeomLoad( in1GEOM *G, CHAR *FileName )
{
  ...
  size = 
    ...
    sizeof(in1UV) * vtn +                     /*   текстурные координаты vtn */
  ...
  IN1_DefaultColor = IN1_ColorSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT minv, maxv, j;
    in1PRIM prim;
    ...
    IN1_PrimCreate(&prim, IN1_PRIM_TRIMESH, vn, fn * 3);
